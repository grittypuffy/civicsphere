    ...
    try:
        ...
        if not user_prefs or "language" not in user_prefs:
            return JSONResponse(
                status_code=400,
                content=CreatePostResponse(
                    success=False, message="User preferences not found"
                ).dict(),
            )
        ...
        # Response moderation
        try:
            func_payload = {"title": title, "description": description}
            func_response = None
        ...
        return JSONResponse(
            status_code=500,
            content=CreatePostResponse(
                success=False, message=f"An internal error occurred: {str(e)}"
            ).dict(),
        )


@router.post("/post/voice", response_model=CreatePostResponse)
        ...
        try:
            ...
            if transcription is None:
                return JSONResponse(
                    status_code=400,
                    content=CreatePostResponse(
                        success=False, message="No transcriptions found"
                    ).dict(),
                )
...
@router.get("/posts")
async def get_community_posts(community_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
        ...
        posts_cursor = config.db["posts"].find({"community_id": community_id})
        ...
        return TrendingResponse(
            success=True, message="Community posts fetched successfully", data=posts
        )
...
async def get_post(community_id: str, post_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
        ...
        if not post:
            return JSONResponse(
                status_code=404, content={"success": False, "message": "Post not found"}
            )
...
async def translate_post(community_id: str, post_id: str, req: Request):
    try:
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
                status_code=401,
        ...
        user = await config.db["userPreferences"].find_one(
            {"user_id": req.state.user.get("user_id")}
        )
...
async def explain_post(community_id: str, post_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
...
async def upvote_post(community_id: str, post_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
        ...
        await config.db["posts"].update_one(
            {"_id": ObjectId(post_id)}, {"$set": {"upvote": updated_vote}}
        )

        await config.db["postReaction"].insert_one(
            {"post_id": post_id, "user_id": req.state.user["user_id"], "upvote": True}
        )
...
async def downvote_post(community_id: str, post_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
        ...
        await config.db["posts"].update_one(
            {"_id": ObjectId(post_id)}, {"$set": {"downvote": updated_vote}}
        )

        await config.db["postReaction"].insert_one(
            {"post_id": post_id, "user_id": req.state.user["user_id"], "upvote": False}
        )
...
async def remove_upvote_post(community_id: str, post_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
...
async def remove_downvote_post(community_id: str, post_id: str, req: Request):
    try:
        # Auth check
        if not hasattr(req.state, "user") or not req.state.user:
            return JSONResponse(
