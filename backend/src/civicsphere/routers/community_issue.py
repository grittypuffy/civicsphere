...
from ..models.api.issue import IssueResponseSingle,Issue, CreateIssueRequest, IssueResponse
...
async def get_community_issue(
    ...
    try:
        ...
        return IssueResponse(
            success=True,
            message="Community issues fetched successfully",
            data=issues
        )
...
async def resolve_issue(
    ...
    try:
        ...
        if issue.get("status") == "Open":
            ...
            return JSONResponse(
                status_code=400,
                content={"success": False, "message": "Only open issues can be resolved"}
            )
...
async def close_issue(
    ...
    try:
        ...
        if issue.get("status") == "Resolved" and req.state.user["user_id"] == issue.get("user_id"):
            await config.db["issue"].update_one(
                {"_id": ObjectId(issue_id)},
                {"$set": {"status": "Closed"}}
            )
            ...
            return JSONResponse(
                status_code=400,
                content={"success": False, "message": "Only the issue creator can close a resolved issue"}
            )
